...
func (s *Integration) TemplateFuncMap(ctx context.Context, config *label.Config) integration.FuncMap {
	return integration.FuncMap{
		...
		"get_product": func(slug string) (Product, error) {
			return s.store.Products.GetPublishedBySlugAndOrganizationID(ctx, slug, config.OrganizationID)
		},
	}
}
	...
	return integration.PresetMap{
		"product": {
			...
			Load: func(params ...string) (any, error) {
				...
				product, err := s.store.Products.GetPublishedBySlugAndOrganizationID(ctx, params[0], config.OrganizationID)
				if err != nil {
		...
		"products": {
			...
			Load: func(params ...string) (any, error) {
				...
				if err != nil {
					return nil, coderror.Newf("products_not_found", "products not found: %w", err)
				}
